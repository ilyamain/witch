...
if ($_POST['p']['form'] == 'issue') 
{
	...
	$bill_example = array(
		...
		'denomination' => number_format($newdenom, 8, '.', ''),
...
if ($_POST['p']['form'] == 'bco') 
{
	...
	if (!empty($bill)) 
	{
		...
		(
			...
			'key' => $newpassword, 
			'algorithm' => $_POST['p']['algo'], 
			'denomination' => number_format($bill['denomination'], 8, '.', ''),
			'timestamp' => $_POST['p']['notime'] ? 0 : gmdate('U'),
		);
		$encrypt = new cEncrypt($bill_example);
		$bill_example['sign'] = $encrypt->sign;
...
if ($_POST['p']['form'] == 'bu') 
{
	...
	if ((!empty($bill1))&&(!empty($bill2))) 
	{
		...
		(
			...
			'key' => $newpassword, 
			'algorithm' => $_POST['p']['algo'], 
			'denomination' => number_format(($bill1['denomination']+$bill2['denomination']), 8, '.', ''),
			'timestamp' => $_POST['p']['notime'] ? 0 : gmdate('U'),
		);
		$encrypt = new cEncrypt($bill_example);
		$bill_example['sign'] = $encrypt->sign;
	...
	if (!empty($bill)) 
	{
		...
		(
			[
				...
				'key' => $newpassword1, 
				'algorithm' => $_POST['p']['algo'], 
				'denomination' => number_format($_POST['p']['newdenom-01'], 8, '.', ''),
				'timestamp' => $_POST['p']['notime'] ? 0 : gmdate('U'),
			],
			[
				...
				'key' => $newpassword2, 
				'algorithm' => $_POST['p']['algo'], 
				'denomination' => number_format($_POST['p']['newdenom-02'], 8, '.', ''),
				'timestamp' => $_POST['p']['notime'] ? 0 : gmdate('U'),
			],
		);
		$encrypt = array
