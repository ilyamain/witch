...
if (is_file(way(__DIR__.DS.'functions.php'))) require_once(way(__DIR__.DS.'functions.php'));
if (is_file(way(__DIR__.DS.'issue.php'))) require_once(way(__DIR__.DS.'issue.php'));
if (is_file(way(__DIR__.DS.'intention.php'))) require_once(way(__DIR__.DS.'intention.php'));
...
class cTransactions 
{
	...
	public function bill_change_owner ($number, $old_key, $new_sign, $new_algorithm, $new_timestamp, $compile = false)
	{
		if ((!is_string($number))||(!is_string($old_key))||(!is_string($new_sign))||(!is_string($new_algorithm))||(!is_timestamp($new_timestamp))) 
		{
		...
		else 
		{
			...
			if (!$wrong_items) 
			{
				...
				if ($compile) 
				{
					...
					$base->bill_change_owner($number, $new_sign, $new_algorithm, $new_timestamp);
					return true;
				}
				else 
				{
	...
	public function bill_unite ($arBills, $number, $sign, $algorithm, $timestamp, $compile = false)
	{
		if ((!is_array($arBills))||(!is_string($number))||(!is_string($sign))||(!is_string($algorithm))||(!is_timestamp($timestamp))) 
		{
			console_line('Невозможно прочитать транзакцию. Неправильный формат.', 2, 'error');
			return false;
		}
	...
	public function bill_split ($old_number, $old_key, $arBills, $timestamp, $compile = false)
	{
		if ((!is_string($old_number))||(!is_string($old_key))||(!is_array($arBills))||(!is_timestamp($timestamp))) 
		{
			console_line('Невозможно прочитать транзакцию. Неправильный формат.', 2, 'error');
			return false;
		}
	...
	public function bill_resort ($arOld, $arNew, $timestamp, $compile = false)
	{
		if ((!is_array($arOld))||(!is_array($arNew))||(!is_timestamp($timestamp))) 
		{
			console_line('Невозможно прочитать транзакцию. Неправильный формат.', 2, 'error');
			return false;
		}
			...
			foreach ($arOld as $iOld) 
			{
				...
				if (!empty($old)) 
				{
					...
					else
					{
						...
						console_line($current['intention'], 2, 'success');
						$current['pubkey'] = (new cEncrypt($current['example']))->pubkey;
						$current['in_list'] = false;
						foreach ($arIntentions as $iIntentions) 
						{
