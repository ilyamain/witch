...
function tally_indicators ($input, $block_id)
{
	...
	(
		...
		2000, // сила транзакций (в номиналах банкнот)
		...
		40,   // номинал эмитируемой банкноты
	);
	...
	$output[8] = ($encrypt_score-$caliber[8])/min($encrypt_score, $caliber[8]);
	// Выравнивающие баллы по комиссионным
	$output[9] = $caliber[9]*($caliber[9]+$input[9])/((1+abs($caliber[9]-$input[9]))*(1+max($input[3], $input[4])));
