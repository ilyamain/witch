...
// $modules = new cModules;
// write($modules->list_full, 100, 'array');
// Если нужно вывести в консоль список всех включенных модулей, раскомментируйте следующую строку
// $modules = new cModules;
// write($modules->list_enabled, 100, 'array');
// Если нужно проверить установлен ли модуль, раскомментируйте следующую строку (либо по id либо по названию модуля)
// $modules = new cModules;
// if ($modules->is_enabled('blocks')) write('enabled', 100, 'array'); else  write('disabled', 100, 'array');
// if ($modules->is_enabled('Blocks', true)) write('enabled', 100, 'array'); else  write('disabled', 100, 'array');
		...
		foreach ($arScan as $iScan) 
		{
			...
			if ((is_dir($module_folder))&&($iScan != '.')&&($iScan != '..')) 
			{
				...
				if (is_file($info_file)) 
				{
					...
					if (is_file($module_folder.DS.'module.install.php')) 
					{
						$module_info['install'] = true;
						$module_info['enabled'] = (is_file($module_folder.DS.'module.config.php')) ? true : false;
					}
					else 
					{
